...
import { Settings as SettingsIcon, Shield, Bell, Palette, Database, Key, Globe, HelpCircle, LogOut, ChevronRight, Moon, Zap, MessageSquare, CreditCard } from 'lucide-react'
...
function SettingRow({ item, onToggle }: { item: SettingItem; onToggle?: () => void }) {
  return (
    <div
      ...
      className={`w-full flex items-center gap-3.5 px-4 py-3.5 hover:bg-white/[0.03] transition-colors rounded-lg ${item.action !== 'toggle' ? 'cursor-pointer' : ''}`}
    >
      ...
      {item.action === 'toggle' && (
        <Toggle value={item.value as boolean} onToggle={onToggle} />
      )}
  ...
  return (
    <div className="px-4 py-4 pb-24 max-w-lg mx-auto space-y-6">
      ...
      <motion.div 
        ...
        className="glass-card rounded-2xl p-5 flex items-center gap-4 border-blue-500/10 neon-glow-blue"
      >
        <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center border-2 border-white/10 shadow-lg" style={{ boxShadow: '0 0 25px rgba(59,130,246,0.3)' }}>
      ...
      <motion.button 
        whileTap={{ scale: 0.98 }}
        className="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl bg-red-500/8 text-red-400 border border-red-500/15 text-sm font-bold hover:bg-red-500/15 transition-colors"
      >
